...
import {
  getPendingExpenses, getExpenseCategories, getProperties, getPayers,
} from '../api/index.js'
...
export default function PendingExpenses({ onSaved, onCountChange, filterType, filterSource }) {
  ...
  const filteredItems = useMemo(() => {
    ...
    if (filterType === EMAIL_TYPE.INCOME) result = result.filter(i => i.emailType === EMAIL_TYPE.INCOME)
    else if (filterType === EMAIL_TYPE.EXPENSE) result = result.filter(i => i.emailType !== EMAIL_TYPE.INCOME)
    return result
  ...
  const emptyMessage = filterSource === EXPENSE_SOURCE.RECEIPT ? 'No pending receipt entries'
    : filterType === EMAIL_TYPE.INCOME ? 'No pending income'
    : filterType === EMAIL_TYPE.EXPENSE ? 'No pending expenses'
    : 'No pending items'

  const contextNote = filterType === EMAIL_TYPE.INCOME ? 'Showing income items only.'
    : filterType === EMAIL_TYPE.EXPENSE ? 'Showing expense items only.'
    : filterSource === EXPENSE_SOURCE.RECEIPT ? 'Showing receipt items only.'
    : null
